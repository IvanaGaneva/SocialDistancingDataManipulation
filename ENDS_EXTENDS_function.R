...
ENDS_EXTENDS_function <- function(data_measures = COVID_measures_df_REVIEWED,
  ...
  if(nrow(state_measures) != 0){
    ...
    if(l != 0){
      ...
      for(i in 1:l){
        ...
        coords <- which(colnames(state_measures) %in% c('Extends', 'Expands',
                                                        'Eases', 'Joins', 'Leaves'))
  ...
  # i.e. if there are no instances of this policy measure within the particular state
  # take for example Alabama, Quarantine (as of 24/04)
  # nothing happens
